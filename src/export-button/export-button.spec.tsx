...
test("render export table button as part of a paged table without selectable rows or fetchBulkExportRows", () => {
  ...
  expect(
    queryByTestId("visible-rows-data-source-radio-option"),
  ).not.toBeInTheDocument();
  expect(
    queryByTestId("bulk-rows-data-source-radio-option"),
  ).not.toBeInTheDocument();
  expect(
    queryByTestId("selected-rows-data-source-radio-option"),
  ).not.toBeInTheDocument();
...
test("render export table button as part of a paged table with selectable rows", () => {
  const { container, getByTestId, queryByTestId } = render(
    <TestWrapper>
      <ExportButton
        ...
        visibleRows={data}
        filenamePrefix="example-data"
        selectedRows={["First Datum", "Fourth Datum"] as any}
      />
    </TestWrapper>,
  );
  expect(container).toMatchSnapshot();
  ...
  expect(
    getByTestId("visible-rows-data-source-radio-option"),
  ).toBeInTheDocument();
  expect(
    queryByTestId("bulk-rows-data-source-radio-option"),
  ).not.toBeInTheDocument();
  expect(
    getByTestId("selected-rows-data-source-radio-option"),
  ).toBeInTheDocument();
