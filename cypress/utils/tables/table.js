...
export const checkSorting = (firstWord, lastWord, descending) => {
  ...
  cy.get("td.word-class").first().should("contain", firstWord);
  cy.get("td.word-class").last().should("contain", lastWord);
};

export const testSelectOne = (shouldStatement) => {
  cy.get("span[data-testid='paged-table-checkbox-cell']").first().click();
...
export const checkFirstWord = (word) => {
  cy.get("td.word-class").first().should("contain", word);
};
...
export const checkTablePager = (display) => {
  cy.get("div.table-pager").find("span").should("contain", display);
};
